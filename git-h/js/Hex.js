function Hex(sideLength) {
	...
	try { currentThemeId = localStorage.getItem('selected_theme') || 'classic'; } catch(e){}
	var theme = (window.themes && window.themes.find(function(t) { return t.id === currentThemeId; })) || { hexFill: [44, 62, 80] };
	this.fillColor = theme.hexFill;
	this.tempColor = theme.hexFill;
		...
		if (position !== undefined) {
			...
			if (position <= 0) {
				if (block.distFromHex - block.iter * this.dt * settings.scale - (this.sideLength / 2) * Math.sqrt(3) <= 0) {
					...
					block.settled = 0;
					var wave = (window.gameMode === 'double') ? (this === window.LeftHex ? window.waveone_left : window.waveone_right) : waveone;
	...
	this.rotate = function(steps, force) {
		if (!force && Date.now()-this.lastRotate<75 && !(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) ) return;
		this.lastRotate = Date.now();

		if (window.gameMode === 'double' && (this === window.MainHex || this === MainHex)) {
			if (window.LeftHex) window.LeftHex.rotate(steps, true);
			if (window.RightHex) window.RightHex.rotate(steps, true);
			return;
		}
	...
	this.draw = function() {
		if (window.gameMode === 'double') {
			if (this === window.LeftHex) {
				this.x = trueCanvas.width * 0.28;
			} else if (this === window.RightHex) {
				this.x = trueCanvas.width * 0.72;
			} else {
				this.x = trueCanvas.width/2;
			}
		} else {
