function blockDestroyedForHex(hex) {
	var wave = (window.gameMode === 'double') ? (hex === window.LeftHex ? window.waveone_left : window.waveone_right) : waveone;
	var limit = window.gameMode === 'double' ? 1500 : 600;
	if (wave.nextGen > 1350) {
		wave.nextGen -= (window.gameMode === 'double' ? 10 : 30) * settings.creationSpeedModifier;
	} else if (wave.nextGen > limit) {
		wave.nextGen -= (window.gameMode === 'double' ? 3 : 8) * settings.creationSpeedModifier;
	} else {
		wave.nextGen = limit;
	}

	var maxDiff = window.gameMode === 'double' ? 12 : 35;
	if (wave.difficulty < maxDiff) {
		wave.difficulty += (window.gameMode === 'double' ? 0.02 : 0.085) * settings.speedModifier;
	} else {
		wave.difficulty = maxDiff;
	}
}

function blockDestroyed() {
	blockDestroyedForHex(window.MainHex);
}

function waveGen(hex) {
	this.lastGen = 0;
	this.last = 0;
	this.nextGen = window.gameMode === 'double' ? 3800 : 2700;
	this.start = 0;
	this.colors = colors;
	this.ct = 0;
	this.hex = hex;
	...
	this.update = function() {
		...
		this.dt = (settings.platform == 'mobile' ? 14 : 16.6667) * this.hex.ct;
		this.computeDifficulty();
		if ((this.dt - this.lastGen) * settings.creationSpeedModifier > this.nextGen) {
			var limit = window.gameMode === 'double' ? 1500 : 600;
			var decrementRate = window.gameMode === 'double' ? 4 : 11;
			if (this.nextGen > limit) {
				this.nextGen -= decrementRate * ((this.nextGen / 1300)) * settings.creationSpeedModifier;
			}
		}
	};
	...
	this.randomGeneration = function() {
		if (this.dt - this.lastGen > this.nextGen) {
			...
			var baseSpeed = window.gameMode === 'double' ? 1.0 : 1.6;
			var speed = baseSpeed + (this.difficulty / 15) * (window.gameMode === 'double' ? 1.5 : 3.0);
			if (window.gameMode === 'double') {
				addNewBlockForHex(fv, colors[randInt(0, colors.length)], speed, this.hex);
			} else {
				addNewBlock(fv, colors[randInt(0, colors.length)], speed);
			}
			var lim = 5;
			if (window.gameMode === 'double') {
				// Don't switch functions in double mode
			} else if (this.ct > lim) {
	...
	this.computeDifficulty = function() {
		if (this.difficulty < (window.gameMode === 'double' ? 12 : 35)) {
			var increment;
			...
			this.difficulty += increment * (window.gameMode === 'double' ? 1/5 : 1/2);
		}
	};
